...
static void process_marithmetic_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    ArithType type = ARITH_INCR; // default mode is to increment.
    bool locked = false;
    uint32_t hv = 0;
    ...
    bool item_created = false;
    switch(do_add_delta(c, key, nkey, type, of.delta, tmpbuf, &of.req_cas_id, hv, &it)) {
    case OK:
        if (c->noreply)
            resp->skip = true;
        if (settings.meta_response_old) {
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, ArithType type) {
    ...
    switch(add_delta(c, key, nkey, type, delta, temp, NULL)) {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "delete") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmetic_command(c, tokens, ntokens, ARITH_DIV);
#ifdef MEMCACHED_DEBUG
